...
{
	ENTITYREG(SceneEntity, CutSceneSendJoe, "Overkill/CutScene", "CutSceneSendJoe")

	META_DATA_DESC(CutSceneSendJoe)
		BASE_SCENE_ENTITY_PROP(CutSceneSendJoe)

	META_DATA_DESC_END()

	eastl::string CutSceneSendJoe::dialogTexts[] =
	{
		"SendJoe_Dialog1",		
		"SendJoe_Dialog2",
		"SendJoe_Dialog3",		
	...
	void CutSceneSendJoe::Init()
	{
		ScriptEntity2D::Init();

		Tasks(true)->AddTask(10, this, (Object::Delegate)&CutSceneSendJoe::Draw);
	}

	void CutSceneSendJoe::Play()
	{
		ScriptEntity2D::Play();
	...
	void CutSceneSendJoe::SetNextDialog(SpriteEntity* dialog)
	{
		curDialog++;
	...
	void CutSceneSendJoe::ShowDialogText(float dt)
	{
	...
	void CutSceneSendJoe::InitializeStars(Node2D* root)
	{
		starBrights.clear();
	...
	void CutSceneSendJoe::UpdateStarBrights(float dt)
	{
	...
	void CutSceneSendJoe::NextScene()
	{		
		curScene++;
		...
		if (curScene == 6)
		{
			WorldManager::instance->SetNextLevel("Level_05", true);
	...
	void CutSceneSendJoe::Draw(float dt)
	{
